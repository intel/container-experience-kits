...
import argparse
import os
from render_util.common.common import create_dir_idempotent, render, load_config, add_arch_parameter, add_nic_parameter, add_mirrors_parameter, create_backups
...
def render_k8s_profiles(args: argparse.Namespace) -> None:
    ...
    add_mirrors_parameter(k8s_profiles, args)

    # create example directory with all profiles and its files
...
def _create_k8s_examples(profiles: dict, args: argparse.Namespace) -> None:
    ...
    only files for the specific profile will be generated into project root directory"""
    if args.profile == 'all_examples':
