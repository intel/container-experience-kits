...
def verify_cwdf_config(config):
    ...
    try:
        ...
        if req.status_code == 200:
            ...
            click.secho(
                "The server https://ident.me is not responding properly.",
                err=True,
                bold=True,
                fg="red")
            click.secho(
                "Unable to find your IP address.",
                err=True,
                fg="red")
...
def compose_cloudcli(
    ...
    cloud_config['ssh_pub_key_path'] = public_key_path
    ...
    if cloud_provider == "aws":
        ...
        click.secho(f"Unknown cloud provider {cloud_provider}.", err=True, bold=True, fg="red")
        click.secho(
            "Currently supported cloud providers are: aws, azure, gpc.",
            err=True,
            bold=True,
            fg="red")
        click.secho("Nothing to do.", err=True, bold=True, fg="red")
